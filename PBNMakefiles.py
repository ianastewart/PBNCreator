...
def get_dd_tricks(deal):
    byte_board = deal.encode(
        "utf-8"
    )  # ddstable requires the board description to be in byte format
...
def CreatePBNfile(folder_path, file_date, no_of_boards, event, site, passedout):
    ...
    if os.path.exists(filepath):
        return (
            ...
            return_hand_data,
            return_dd_data,
        )  # return a '1' in the Event if file already exists
    ...
    file.writelines(
        "%This pbn was generated by Peter Ellington's 'PBN Generator' Python program\n"
    )
    for board_no in range(no_of_boards):
        ...
        deal = gen.generate_random_hand(Dealer, passedout)
        ...
        file.writelines(
            [
                '[DoubleDummyTricks "' + double_dummy_tricks + '"]\n',
