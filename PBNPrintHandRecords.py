...
HCP_width = 7
dd_width = 28
layout_width = left_margin + 2 * (
    board_column_width + dealer_column_width + hands_width + HCP_width + dd_width
)
heading_height = 4
deal_height = 17
top_line = 20
...
# remove any existing pdf files in the temporary directory if possible
...
def print_pbn_file(
    ...
    full_detail = len(seats) == 4
    draw_lines(pdf, no_of_boards, full_detail)
    for board_num in range(no_of_boards):
        ...
        sorted_hands = print_hands(deal, pdf, x_origin, y_origin, seats)
        # HCP
        if full_detail:
...
def draw_lines(pdf, no_of_boards, full_detail=True):
    """This function draws the grid lines on the printed sheets."""
    no_of_rows = int(no_of_boards / 2)
    if no_of_boards % 2 != 0:
        no_of_rows += 1  # if odd number of boards draw and extra row
    # Horizontal lines
    pdf.line(left_margin, top_line, layout_width, top_line)
    pdf.line(
        left_margin, top_line + heading_height, layout_width, top_line + heading_height
    )
...
def PrintHeadings(Site, Event, PlayDate, pdf):
    ...
    for i in range(2):
        ...
        pdf.cell(
            board_column_width, 0, txt="Board", ln=0, align="C"
        )  # cell for 'Board' heading
...
def print_hands(deal, pdf, x_origin, y_origin, seats):
    """seats defines which of NSEW should be printed"""
    ...
    # covert seats string "NSEW" or "NE" etc to list of numbers in range 0 - 3
    seat_list = []
    if "N" in seats:
        seat_list.append(0)
    if "S" in seats:
    ...
    for i in seat_list:
        pdf.set_xy(
            x_origin + board_column_width + dealer_column_width,
            y_origin + (line_separation * i),
        )
        ...
        for j in range(4):
            ...
            if not point == "":  # print out point(N,S,E,W)
                ...
                text = point
                pdf.cell(text_width + 2, line_separation, txt=text, ln=0, align="L")
            # suit symbol
            pdf.set_font("Symbol", size=symbol_font_size)
            text = suit_chars[j]
            # set colour of heart or diamond symbol to red
            if j == 1 or j == 2:
                pdf.set_text_color(255, 0, 0)
            else:
                pdf.set_text_color(0, 0, 0)
            width = symbol_width
            pdf.cell(width, line_separation, txt=text, ln=0, align="L")
            # cards in suit
            pdf.set_font("Helvetica", size=font_size)
            text = suits[j]
            if text == "":
                text = "-"
            width = text_width * len(text)
            pdf.set_text_color(0, 0, 0)
            line = 0
            if j == 3 and i != 3:
                line = 1  # print out 4 suits before a new line
...
def PrintDoubleDummy(ddtricks, pdf, x_origin, y_origin):
    ...
    dd_origin = (
        x_origin + board_column_width + dealer_column_width + hands_width + HCP_width
    )
