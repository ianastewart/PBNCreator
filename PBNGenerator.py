...
def GenFiles():
    ...
    if event == "" or site == "":
        messagebox.showerror(
            title="Error", message="You must enter the Site and Event."
        )
        return
    ...
    if end_date < start_date:
        messagebox.showerror(
            title="Error", message="The end date is less than the start date."
        )
    ...
    if no_of_days == 0:
        ...
        pb_increment = abs(
            100 / (no_of_days / 7)
        )  # increment of progress bar required for each week between start and end date

    while current_date <= end_date:
        # Create the PBN file for the given date and return headings, hand and double dummy data for printing
        return_heading_data, return_hand_data, return_dd_data = makefiles.CreatePBNfile(
            folder_path, current_date, no_of_boards, event, site, passedout
        )  # generate the pbn file
        if (
            return_heading_data["Event"] == "X"
        ):  # if an error occured when trying to open the PBN file an X is place in the Event name
            messagebox.showerror(
                "Error", "pbn file already exists - generation abandoned"
            )
            return
        if Pdf_file:
    ...
    if Pdf_file:
        ...
        os.startfile(
            f
        )  # open the pdf file in default pdf application (Normally Adobe Acrobat)
...
def settings():
    ...
    passed_out = IntVar()
    settingspassed_out_field = Checkbutton(
        settingswin,
        text="Reject boards where no hand has more than 11 points",
        variable=passed_out,
    )
    ...
    OKbutton = Button(
        ...
        text="OK",
        width=7,
        command=lambda: settingsOK(
            settingswin,
            settingssite_field.get(),
            settingsevent_field.get(),
            settingsboards_field.get(),
            passed_out.get(),
        ),
    )
    ...
    Cancelbutton = Button(
        settingswin, text="Cancel", width=7, command=lambda: settingswin.destroy()
    )
    ...
    if passedout == "1":
        settingspassed_out_field.select()
...
def Help():
    ...
    Helptext.insert(
        END,
        "   1. Create a folder into which the pbn deal files are to be stored.\n\n",
        "normal",
    )
    Helptext.insert(END, "   2. Start up PBNGenerator app.\n\n", "normal")
    Helptext.insert(
        END, "   3. If this is the first time the app has been used: \n\n", "normal"
    )
    Helptext.insert(
        END, "      \u25CF Call up Settings from the 'Edit' menu.\n\n", "normal"
    )
    Helptext.insert(
        END,
        "      \u25CF Enter the default Site, Event and number of boards to be dealt.\n\n",
        "normal",
    )
    Helptext.insert(END, "      \u25CF Click OK to save.\n\n", "normal")
    Helptext.insert(
        END,
        "   4. Select the start and end dates for which weekly files are to be generated. You can click\n \
       on the downarrow next to the date to bring up a calendar.\n\n",
        "normal",
    )
    Helptext.insert(
        END,
        "   5. Select the day of the week on which the bridge session is to be played.\n\n",
        "normal",
    )
    Helptext.insert(
        END,
        "   6. If you want printed sheets showing the hand records for each of the pbn files select\n      'Print Sheets'.\n\n",
        "normal",
    )
    Helptext.insert(
        END,
        "   7. Click the 'Generate' button.  A 'Select Folder' window will open. Select the folder\n \
       created in step 1 and click OK.\n\n",
        "normal",
    )
    Helptext.insert(
        END,
        "      That's it - the files will be generated and sheets printed to the default printer.\n\n",
        "normal",
    )
    Helptext.insert(
        END,
        "      The names of the files will be in the form 'YYMMDD.pbn' where the playing date is DDMMYYY.\n\n",
        "normal",
    )
    Helptext.insert(
        END,
        "      To produce pbn files for more than one day in the week, change the event and repeat the\n \
    ...
    Helptext.pack()
    Closebutton = Button(
        Helpwin, text="Close", width=7, command=lambda: Helpwin.destroy()
    )
...
def About():
    ...
    TextLabel = Label(
        Aboutwin, textvariable=LabelVariable, justify=LEFT, width=35, wraplength=200
    )
    TextLabel.pack()
    LabelVariable.set(
...
passed_out = (
    IntVar()
)  # option variable = 1 if board with no hand have more than 11 HCP is to be rejected
passedout = str(passed_out)
